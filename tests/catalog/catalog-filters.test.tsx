...
describe('CatalogFiltersNew', () => {
  ...
  beforeEach(() => {
    ...
    Object.keys(mockSearchParams).forEach(key => mockSearchParams.delete(key))
  ...
  it('renders with initial params', () => {
    ...
    expect(screen.getByLabelText('Show in-stock products only')).toBeInTheDocument()
  ...
  it('sets category when category chip is clicked', async () => {
    const user = userEvent.setup()
    render(<CatalogFiltersNew {...defaultProps} />)
  ...
  it('clears category when "All Categories" is clicked', async () => {
    ...
    const propsWithCategory = {
      ...defaultProps,
      initial: { ...defaultProps.initial, category: 'cat-1' },
    }

    render(<CatalogFiltersNew {...propsWithCategory} />)
